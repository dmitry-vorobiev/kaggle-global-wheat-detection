...
def create_dataset(conf, transforms, show_progress=False):
    # type: (DictConfig, DictConfig, Optional[bool]) -> WheatDataset
...
def create_train_loader(conf, rank=None, num_replicas=None):
    ...
    data = create_dataset(conf, conf.transforms, show_progress=show_progress)
