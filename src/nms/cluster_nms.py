...
def diou_ab(box_a: Tensor, box_b: Tensor, gamma=0.9, eps=1e-8):
    """
    Args:
...
def cluster_nms_diou(boxes, scores, thresh=0.5, top_k=200):
    assert boxes.ndim == 2
    ...
    iou = diou_ab(top_k_boxes, top_k_boxes)[0]
    ...
    for i in range(top_k):
        ...
        # keep far away boxes
        keep = (best_iou <= thresh)[:, None].expand_as(iou)
        iou = torch.where(keep, iou, torch.zeros_like(iou))
        if (iou == iou0).all():
            break

    idx = idx[best_iou <= thresh]
